...
export const ROLE_PERMISSIONS: Record<StaffRole, StaffPermission[]> = {
  SUPER_ADMIN: [
    ...
    'ANALYTICS', 'SETTINGS', 'CHAT_SAFETY', 'AI_RULES', 'FINANCE', 'PRODUCTS',
    'CONTENT_PAGES',
  ],
  ADMIN: [
    'MODERATION', 'REPORTS', 'VERIFICATIONS', 'USERS', 'CATEGORIES',
    'AUCTIONS', 'BANNERS', 'FLASH_SALES', 'BOOSTS', 'PLANS', 'NOTIFICATIONS',
    'ANALYTICS', 'SETTINGS', 'CHAT_SAFETY', 'FINANCE', 'PRODUCTS',
    'CONTENT_PAGES',
  ],
  // Mirrors the backend gate on /admin/content-pages, which allows
  // CONTENT_MANAGER, ADMIN and SUPER_ADMIN. Keep the two in step: a nav entry
  // the server refuses is worse than no nav entry at all.
  CONTENT_MANAGER: ['MODERATION', 'CATEGORIES', 'BANNERS', 'FLASH_SALES', 'PRODUCTS', 'CONTENT_PAGES'],
